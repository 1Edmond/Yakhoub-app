...
import 'package:flutter_sixvalley_ecommerce/helper/vendor/localization/language_constrants.dart';
...
class DateConverter {
  ...
  static String formatDateWithComma(DateTime dt) {
  ...
  static String toLocalFormattedDateTime(String dateTime) {
    final localDateTime = DateFormat('yyyy-MM-ddTHH:mm:ss.SSS').parse(dateTime, true).toLocal();
  ...
  static String utcStringToAuctionDateTime(String? dateString) {
    if (dateString == null || dateString.isEmpty) return '';
    try {
      final local = DateTime.parse(dateString).toLocal();
      return DateFormat('d MMM yyyy, hh:mm a').format(local);
    } catch (_) {
      return dateString;
    }
  }

  static String formatAuctionDateTime(DateTime? dateTime) {
    if (dateTime == null) return '';
    return DateFormat('d MMM yyyy, hh:mm a').format(dateTime);
  }

  static String utcStringToAuctionDuration(String? start, String? end) {
    if (start == null || end == null) return '';
    final s = utcStringToAuctionDateTime(start);
    final e = utcStringToAuctionDateTime(end);
    return '$s - $e';
  }
