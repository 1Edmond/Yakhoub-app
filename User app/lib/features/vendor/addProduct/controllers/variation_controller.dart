...
import 'package:flutter_sixvalley_ecommerce/features/vendor/features/addProduct/domain/models/attr.dart';
...
class VariationController extends ChangeNotifier {
  ...
  Future<void> getAttributeList(BuildContext context, Product? product, String language) async {
    ...
    if (response.response != null || response.response!.statusCode != 200) {
      ...
      if (product != null && _attributeList != null && _attributeList!.isNotEmpty) {
        if(addColor==0) {
          addColor++;
          if ( product.colors != null && product.colors!.isNotEmpty) {
            Future.delayed(Duration.zero, () async {
              Provider.of<VariationController>(Get.context!, listen: false).setAttribute();
              Provider.of<VariationController>(Get.context!, listen: false).attributeList![0].variants = [];
            });
          }
          for (int index = 0; index < product.colors!.length; index++) {
            colors.add(index);
            Future.delayed(Duration.zero, () async {
              addVariant(Get.context!, 0, product.colors![index].name, product, false);
              addColorCode(product.colors![index].code, index: index);
            });
          }
        }
      }
