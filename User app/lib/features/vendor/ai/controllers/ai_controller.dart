...
import 'package:flutter_sixvalley_ecommerce/features/vendor/features/ai/domain/models/ai_variation_model.dart';
import 'package:flutter_sixvalley_ecommerce/features/vendor/features/ai/domain/models/genara_setup_model.dart';
import 'package:flutter_sixvalley_ecommerce/features/vendor/features/ai/domain/models/image_response_model.dart';
import 'package:flutter_sixvalley_ecommerce/features/vendor/features/ai/domain/models/pricing_model.dart';
import 'package:flutter_sixvalley_ecommerce/features/vendor/features/ai/domain/models/title_model.dart';
import 'package:flutter_sixvalley_ecommerce/features/vendor/features/ai/domain/models/title_suggestion_model.dart';
import 'package:flutter_sixvalley_ecommerce/features/vendor/features/ai/domain/services/ai_service_interface.dart';
...
class AiController extends ChangeNotifier {
  ...
  bool _auctionBasicSetupLoading = false;
  bool get auctionBasicSetupLoading => _auctionBasicSetupLoading;

  Future<void> generateAuctionPageSetup({
    required String title,
    required TextEditingController descriptionController,
    required String langCode,
    void Function(GeneralSetupModel? model)? onGeneralSetupDone,
  }) async {
    _auctionBasicSetupLoading = true;
    notifyListeners();

    await generateDescription(title: title, langCode: langCode);

    if (_descriptionModel?.data != null && _descriptionModel!.data!.isNotEmpty) {
      descriptionController.text = _descriptionModel!.data!;
      notifyListeners();
    }

    await generateGeneralSetup(title: title, description: descriptionController.text, langCode: langCode);

    onGeneralSetupDone?.call(_generalSetupModel);
    ...
    _auctionBasicSetupLoading = false;
    notifyListeners();
  }
  ...
  Future<void> generateAuctionDescription({
    ...
    ApiResponse apiResponse = await aiServiceInterface.generateAuctionDescription(
      title: title,
      langCode: langCode,
    );
  ...
  Future<void> generateAuctionTitle({
    ...
    ApiResponse apiResponse = await aiServiceInterface.generateAuctionTitle(
      title: title,
      langCode: langCode,
    );
    ...
    if (_titleModel?.data != null && _titleModel!.data!.isNotEmpty) {
      nameController.text = _titleModel!.data!;
      notifyListeners();
    }
  }
  ...
  Future<ImageResponseModel?>? generateAndSetDataFromImage({required XFile image, List<Language>? languageList, TabController? tabController, List<TextEditingController>? nameControllerList  }) async {
    ...
    if(response.response?.statusCode == 200) {
      ...
      if(imageResponseModel.data?.data != null) {
        nameControllerList![tabController?.index ?? 0].text = imageResponseModel.data?.data ?? '';
